...
class GenericLangGraphExecutor(AgentExecutor):
    ...
    This executor follows the Open/Closed Principle - open for extension (new agents)
    but closed for modification (no need to change this class for new agents).
    """

    def __init__(self, agent_class: Type, agent_name: str = "LangGraph Agent"):
...
def create_langgraph_executor(agent_class: Type, agent_name: str = None) -> GenericLangGraphExecutor:
    ...
    return GenericLangGraphExecutor(agent_class, agent_name)


# Example usage:
# from langgraph_data_processor import LangGraphDataProcessor
# executor = create_langgraph_executor(LangGraphDataProcessor, "Data Processor")